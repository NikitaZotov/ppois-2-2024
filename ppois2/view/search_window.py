...
class SearchWindow(Tk):
    ...
    def create_radiobuttons(self):
        label = Label(self, text="Выберите способ поиска:", name='main_label')
    ...
    def diagnosis_entry(self):
        label_diagnosis = Label(self, text="Фраза из диагноза:")
    ...
    def create_button(self):
        button = ttk.Button(self, text='Найти', command=self.find_click)
    ...
    def show_table(self):
        ...
        for widget in self.winfo_children():
            widget.destroy()
        columns = ("Имя", "Дата рождения", "Последнее посещение", "Врач", "Диагноз")
        tree = ttk.Treeview(self, columns=columns, show="headings")
        for col in columns:
            tree.heading(col, text=col)
        for record in self.pet_handler.result:
            tree.insert("", END, values=(record["name"], record["birth_date"],
                                         record["last_visit_date"], record["fio"], record["diagnosis"]))
