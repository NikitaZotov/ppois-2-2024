...
from main.source.fisherman import Fisherman, Experience
...
class FisheryProduction:
        ...
        if self.fishermen and self.ships:
            ...
            while True:
                try:
                    ...
                    fishermen = self.choose_fishermen(fishermen_list)
                    break
                except (IndexError, ValueError):
            ...
            while True:
                try:
                    if self.free_nets:
                        ...
                        self.choose_nets(ship, nets_list)
                    break
                except (IndexError, ValueError):
                    pass
    ...
    def finish_production(self) -> None:
        ...
        util = Util()
        util.save_state("fishery_production.json", self)
    ...
    def choose_ship(self, choice) -> Ship:
        ship = self.ships[int(choice) - 1]
        self.borrowed_ships.append(ship)
        self.ships.remove(ship)
        return ship

    def choose_fishermen(self, fishermen_list) -> List[Fisherman]:
    ...
    def choose_nets(self, ship: Ship, nets_list) -> None:
        if self.free_nets:
