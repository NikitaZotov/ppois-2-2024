...
from main.source.fish import FishState, Fish
...
class ColdStorage:
    ...
    def process_fish(self) -> List[Fish]:
        if self.fish_from_fishing:
            fish_for_return = []
            for fish in self.fish_from_fishing:
                fish.state = FishState.PROCESSED
                self.fish_after_processing.append(fish)
                fish_for_return.append(fish)
            self.fish_from_fishing.clear()
            return fish_for_return
        else:
            return []
    ...
    def freeze_fish(self, target_weight: int) -> List[Fish]:
        total_weight = 0
        fish_for_return = []
        for fish in self.fish_after_processing.copy():
            if total_weight + fish.weight >= target_weight:
    ...
    def sell_fish_to_market(self, weight_limit: int) -> List[Fish]:
        ...
        if fish_to_sell:
            ...
            for fish in fish_to_sell:
                if fish in self.fish_after_processing:
                    self.fish_after_processing.remove(fish)
                if fish in self.frozen_fish:
                    self.frozen_fish.remove(fish)
            return fish_to_sell
        else:
