...
class Controller:
    ...
    def on_search_dialog_ok(self, criteria):
        ...
        messagebox.showinfo("Search Results", f"Found {len(results)} record(s) matching criteria.")
        self.search_results_view = SearchResultsView(self.view, self, results)
        self.search_results_view.transient(self.view)
        self.search_results_view.grab_set()
        self.search_results_view.wait_window()
