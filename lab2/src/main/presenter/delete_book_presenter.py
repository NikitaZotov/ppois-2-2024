...
class DeleteBookPresenter(AbstractPresenter):

    def __init__(self, window: DeleteBookWindow, repo: IBookRepository):
    ...
    def __ok_button_click_handler(self):
        if tkinter.messagebox.askyesno(
                "",
                f"Are you really want to delete {len(self._books)} records?",
                master=self._window):
            for book in self._books:
                self._repo.delete_book(book.title)
    ...
    def __search_by_title(self):
        try:
            self._books.append(self._repo.find_book_by_title(self._window.search_args["title"].get()))
        except RepositoryException:
            tkinter.messagebox.showinfo(
                "Message",
                "Nothing was found",
