...
class DeleteWindow(tk.Toplevel):
    ...
    def init_widgets(self):
        ...
        ttk.Button(self, text="Search", command=self.delete_player).pack(pady=10)
    ...
    def delete_player(self):
        ...
        amount_of_deleted_elements = self.application.controller.delete_players(search)
        if amount_of_deleted_elements > 0:
            messagebox.showinfo("Delete player", f"Successfully deleted {amount_of_deleted_elements} players")
        else:
            messagebox.showinfo("Delete player", "No players found for the given criteria")
